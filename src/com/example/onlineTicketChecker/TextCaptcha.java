...
public class TextCaptcha extends Captcha {
	...
	public  TextCaptcha(int height, int width, int wordLength, TextOptions opt) {
		setHeight(height);
    	setWidth(width);
    	this.options = opt;
	...
	protected Bitmap image() {
	   LinearGradient gradient = new LinearGradient(0, 0, getWidth() / this.wordLength, getHeight() / 2, Color.GREEN, Color.YELLOW, Shader.TileMode.MIRROR);
	    Paint p = new Paint();
	    p.setDither(true);
	    ...
	    Paint tp = new Paint();
	    tp.setDither(true);
	    tp.setTextSize(getWidth() / getHeight() * 5);
